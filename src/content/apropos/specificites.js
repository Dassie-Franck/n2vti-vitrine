export const specificites = {
  tagline: 'PARTICULARITÉS',
  titre: 'Nos spécificités',
  intro: "N2VTI est ouvert au plus grand nombre au Cameroun :",
  image: '/n2vti.png',
  points: [
    "Le volet financier est abordé avec la plus grande attention : des facilités de paiement échelonné sont proposées, mettant famille et apprenants face à leurs responsabilités, sans jamais compromettre l'accès à la formation.",
    "Une place importante est accordée aux jeunes filles et aux femmes en reconversion professionnelle, notamment dans les filières paramédical et beauté/esthétique.",
    "La présence sur deux campus (Yaoundé) facilite les contacts avec les candidats, les apprenants et leurs familles, quel que soit leur lieu de résidence.",
    "N2VTI a mis en œuvre des cours de mise à niveau pour les candidats souhaitant consolider leurs bases avant d'intégrer une formation qualifiante.",
    "L'admission est organisée sur étude de dossier et entretien de motivation, sans concours écrit, pour privilégier la motivation et le potentiel de chaque candidat.",
  ],
}